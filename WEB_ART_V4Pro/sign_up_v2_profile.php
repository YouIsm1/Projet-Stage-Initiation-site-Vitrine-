                ...
                if (in_array($the_email, $email_array)) {
                    ...
                    if ($res_2) {
                       ...
                       if ($the_password === $row_2['password']) {
                         ...
                         $_SESSION["id_user"] = $row_2['id'];
                         
                           // Assuming $row_2['id'] contains the user's ID
                          // $userID = $row_2['id'];
                          //
                          // // Use urlencode() to properly encode the query parameter value
                          // $userIDEncoded = urlencode($userID);

                          // Redirect the user to page_profile.php with the encoded user ID as a query parameter
                          // header("Location: page_profile.php?info_con=True&id=$userIDEncoded");
